...
export const createEvent = mutation({
  args: {
    ...
    hasPro: v.optional(v.boolean()), // only for logic
  },
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (!hasPro && args.themeColor && args.themeColor !== defaultColor) {
        throw new Error("Custom colors are Pro feature.");
      }
      ...
      // ✅ IMPORTANT FIX HERE
      const { hasPro: _ignore, ...eventData } = args;

      const eventId = await ctx.db.insert("events", {
        ...eventData, // ✅ NO hasPro now
        themeColor,
        slug: `${slug}-${Date.now()}`,
